...
class Server():
    ...
    def LeaderElection(self):
        ...
        try:
            ...
            ELsock.send(self.election_message.encode())
            try:
                answer = ELsock.recv(1024)
                answer = answer.decode()
                print("Election Message sent to: {} ".format(neighbor_IP))
            except socket.timeout:
                print("Connection failed: {}".format(neighbor_IP))
